...
public class Player {
	...
	public void spawnEnemies()
	{
		...
		for(double cT = 0; cT < Math.PI/4; cT += Math.PI/30)
		{
			for(int negAndPos = -1; negAndPos < 2; negAndPos += 2)
			{
				...
				int xSpawn = (int) (x + (Math.cos(spawnTheta) * Game.WIDTH/1.5));
				int ySpawn = (int) (y + (Math.sin(spawnTheta) * Game.HEIGHT/1.5));
	...
	public void render(Graphics g) // Update picture for player
	{
		...
		for (Enemy en : map.getEnemies(map.getMap()[(int) x][(int) y].getDif())){

			System.out.println(map.getEnemies(map.getMap()[(int) x][(int) y].getDif()).size());
			System.out.println(en.getStats().getActive());
			if (en.getStats().getActive())
			{
				en.render(g, x, y);
				System.out.println("Rendering enemy from player");
