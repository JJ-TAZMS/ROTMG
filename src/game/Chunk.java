...
public class Chunk {
	...
	public void setTiles(int x, int y)
	{
		for (int r = 0; r < CHUNKSIZE; r++)
		{
			...
			{
				tiles[r][c] = new Tile(x*CHUNKSIZE*Tile.TILESIZE + r*(Tile.TILESIZE), y*CHUNKSIZE*Tile.TILESIZE + c*(Tile.TILESIZE), count);
			}
		}
	}
	...
	public void renderNoRot(Graphics g, double xIn, double yIn, double theta)
	{
		...
		for (int r = 0; r < CHUNKSIZE; r++)
		{
			for (int c = 0; c < CHUNKSIZE; c++)
			{
				if (tiles[r][c].getObst() != null)
				{
					tiles[r][c].renderNoRot(g, xIn, yIn, theta);
				}
			}
		}
	}
